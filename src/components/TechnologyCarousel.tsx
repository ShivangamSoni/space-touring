...
export default function TechnologyCarousel({
  ...
  return (
    <div className="grid grid-rows-[auto_1fr] lg:grid-rows-none lg:grid-cols-2 gap-6 lg:gap-10 items-center">
      <Image src={technology.images.landscape} alt="" className="lg:hidden" />

      <div className="grid items-start lg:grid-cols-[auto_1fr] gap-10 lg:gap-16">
        <ul className="flex lg:flex-col items-center justify-center gap-4">
          {technologies.map(({ id, name }, idx) => {
            ...
            return (
              <li key={id}>
                <button
                  className={`outline-none w-16 lg:w-20 aspect-square rounded-full text-2xl lg:text-3xl border border-gray-500 ${
                    isActive
        ...
        <div className="grid gap-8 text-center">
          <h3 className="flex flex-col gap-1 uppercase tracking-widest">
            ...
            <span
              className={`${bellfair.className} text-4xl lg:text-5xl text-white`}
            >
              {technology.name}
            </span>
          </h3>
          <p className="lg:text-justify text-lg tracking-wider w-4/5 mx-auto lg:w-full lg:mx-0">
